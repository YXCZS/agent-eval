...
export function AgentsView() {
  ...
  function saveVersion() {
    ...
    if (isNew) { const newAgent = { ...draft, id: `agent-local-${Date.now()}`, name: draft.name.trim(), versions: draft.versions }; setAgents((current) => [newAgent, ...current]); setSelectedId(newAgent.id); setDraft(newAgent); setIsNew(false); setNotice("Agent 已创建到当前工作区。"); }
    else { setAgents((current) => current.map((agent) => agent.id === draft.id ? draft : agent)); setNotice(`草稿 ${currentVersion?.label ?? "版本"} 已保存到本地。通过 API 持久化后，新版本将保持不可变。`); }
    setConnection(null);
  }
  ...
  async function testConnection() {
    ...
    try { responseFormat = schemaText.trim() ? JSON.parse(schemaText) as Record<string, unknown> : null; } catch { setConnection({ tone: "danger", message: "响应架构不是有效的 JSON。", detail: "测试前请先修正架构。" }); return; }
...
function EndpointFields({ config, update, type }: { config: EndpointConfig; update: (field: keyof EndpointConfig, value: string | number) => void; type: AgentType }) {
  return <div className="config-section"><div className="config-section-title"><Globe2 size={15} /><strong>HTTP 适配器</strong><span>{typeLabels[type]} /run 协议</span></div><label className="field-label">Agent 端点<input value={config.url} onChange={(event) => update("url", event.target.value)} placeholder="https://your-agent.example.com/run" /></label><div className="field-grid two"><label className="field-label">认证引用<input value={config.auth_ref} onChange={(event) => update("auth_ref", event.target.value)} placeholder="project-agent-key" /></label><label className="field-label">协议版本<input value={config.protocol_version} onChange={(event) => update("protocol_version", event.target.value)} /></label></div><div className="field-grid three"><label className="field-label">超时时间（秒）<input type="number" min="1" value={config.timeout_seconds} onChange={(event) => update("timeout_seconds", Number(event.target.value))} /></label><label className="field-label">最大工具调用次数<input type="number" min="0" value={config.max_tool_calls} onChange={(event) => update("max_tool_calls", Number(event.target.value))} /></label><label className="field-label">重试次数<input type="number" min="0" max="5" value={config.max_retries} onChange={(event) => update("max_retries", Number(event.target.value))} /></label></div></div>;
}
